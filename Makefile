...
check_abci_specs:
	autonomy analyse abci generate-app-specs packages.valory.skills.keep3r_job.rounds.Keep3rJobAbciApp packages/valory/skills/keep3r_job/fsm_specification.yaml || (echo "Failed to check job abci consistency" && exit 1)
	autonomy analyse abci generate-app-specs packages.valory.skills.keep3r_abci.composition.Keep3rAbciApp packages/valory/skills/keep3r_abci/fsm_specification.yaml || (echo "Failed to check chained abci consistency" && exit 1)
