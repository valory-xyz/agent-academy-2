...
"""Tests for the Yearn Factory Harvest Job contract."""
...
from packages.valory.contracts.yearn_factory_harvest_job.contract import (
    YearnFactoryHarvestJobContract,
)


ENDPOINT_GANACHE_URI = f"{DEFAULT_GANACHE_ADDR}:{DEFAULT_GANACHE_PORT}"
RECEIPT_TIMEOUT = 30
YEARN_FACTORY_HARVEST_JOB_ADDRESS = "0xEC771dc7Bd0aA67a10b1aF124B9b9a0DC4aF5F9B"


@pytest.mark.skip(reason="The fork freezes because of the amount of reqs to the node.")
@skip_docker_tests
class TestYearnFactoryHarvestJobContract(BaseContractTestCase):
    """Test YearnFactoryHarvestJobContract contract."""

    contract_address = YEARN_FACTORY_HARVEST_JOB_ADDRESS
    path_to_contract = Path(__file__).parent
    ledger_identifier = EthereumCrypto.identifier
    contract: YearnFactoryHarvestJobContract

    # ganache fork configuration
    NETWORK_TO_FORK = "mainnet"
    BLOCK_TO_FORK_FROM = 16838171
